...
class TestDeviceList(base.TestCase):
    ...
    def test_arguments(self, m_super):
        ...
        m_base = mock.patch.object(dl.DeviceListSerial, "__bases__", f_bases)
        with m_base:
    ...
    def test_take_action(self, m_dev_obj):
        ...
        with m_base:
            ...
            self.assertEqual(t_result[1][0][6], m_mod_dev.port)
            self.assertEqual(t_result[1][0][7], m_mod_dev.baudrate)
            self.assertEqual(t_result[1][0][8], m_mod_dev.bytesize)
            self.assertEqual(t_result[1][0][9], m_mod_dev.parity)
            self.assertEqual(t_result[1][0][10], m_mod_dev.stopbits)
            self.assertEqual(t_result[1][0][11], m_mod_dev.timeout)
